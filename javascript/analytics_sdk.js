...
const SDK = function () {
  ...
  this.logEvent = async function (event) {
    this.queue.push(event);
  };

  this.send = async function () {
    if (this.queue.length === 0) return;
    const eventToBeLogged = this.queue.shift();
    try {
      await wait.call(this);
      console.log("Analytics sent ", eventToBeLogged);
    } catch (error) {
      console.log("-----------------------");
      console.log("Failed to send ", eventToBeLogged);
      console.log("Retrying sending ", eventToBeLogged);
      console.log("-----------------------");
      this.queue.unshift(eventToBeLogged);
    } finally {
      this.send();
    }
  };
